    ...
    <div className="flex bg-slate-50 min-h-screen">
      ...
      <main className="flex-1 p-10 overflow-y-auto">
        <div className="max-w-5xl mx-auto">
          <h1 className="text-lg font-black mb-8 tracking-tight text-slate-900 uppercase">Purchase History</h1>
             ...
             {orders.length > 0 ? orders.map((order: any) => (
                <div key={order._id} className="bg-white border border-slate-200 rounded-[2rem] p-8 shadow-sm hover:shadow-lg transition-all group text-slate-900">
                   <div className="flex flex-col md:flex-row justify-between gap-4 mb-6 pb-6 border-b border-slate-50">
                      <div>
                         <p className="text-[8px] font-black text-indigo-600 uppercase tracking-widest mb-1 px-2 py-0.5 bg-indigo-50 rounded-full w-fit">Ref: {order._id.slice(-6).toUpperCase()}</p>
                         <h3 className="text-base font-black uppercase tracking-tight">Status: {order.status}</h3>
                         <p className="text-slate-400 text-[9px] font-bold italic mt-0.5 uppercase tracking-widest">{new Date(order.createdAt).toLocaleDateString()}</p>
                      </div>
                      <div className="flex flex-col items-end gap-1">
                         <span className="text-xl font-black tracking-tighter">{order.total.toLocaleString()} TK</span>
                      ...
                      <div className="space-y-3">
                         ...
                         {order.items?.map((item: any) => (
                            <div key={item._id} className="flex items-center gap-3">
                               ...
                               <p className="text-[10px] font-black uppercase tracking-tight text-slate-700">{item.productId?.name} x {item.quantity}</p>
                            </div>
                         ))}
                      </div>
                      <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100">
                         <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-2">Location</p>
                         <p className="text-[10px] font-bold text-slate-900 leading-tight uppercase tracking-tight">{order.shippingAddress?.street}</p>
                      </div>
                   </div>

                   <div className="flex gap-3">
                      <Link href={`/orders/${order._id}`} className="px-5 py-2.5 bg-white border border-slate-200 rounded-xl font-black text-[9px] uppercase tracking-widest hover:bg-slate-50 transition-all shadow-sm">Details</Link>
                      {(order.status === 'Pending' || order.status === 'Processing') && (
                         <button 
                            onClick={() => handleCancel(order._id)}
                            disabled={isCancelling}
                            className="px-5 py-2.5 bg-red-50 text-red-600 rounded-xl font-black text-[9px] uppercase tracking-widest hover:bg-red-100 transition-all"
                         >Terminate</button>
                      )}
                   </div>
                </div>
