  ...
  return (
    <>
      <header className="mb-8">
        <h1 className="text-lg font-black tracking-tight mb-1 text-slate-900 uppercase leading-none">System Architecture</h1>
        <p className="text-slate-500 font-bold italic text-[10px] tracking-wide">Infrastructure health and environmental telemetry.</p>
      </header>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <section className="bg-slate-900 rounded-3xl p-8 shadow-xl shadow-slate-900/20 text-white">
           <h2 className="text-[10px] text-white/40 uppercase tracking-[0.3em] font-black mb-6">Unit Health</h2>
           <div className="space-y-6">
              {[
                ...
                <div key={i} className="flex items-center justify-between">
                   <span className="text-[9px] uppercase tracking-widest font-bold text-white/60">{item.name}</span>
                   <span className="text-[8px] font-black text-green-400 uppercase tracking-widest">{item.status}</span>
                </div>
              ))}
           </div>
        </section>
