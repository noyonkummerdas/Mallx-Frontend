  ...
  return (
    <>
      <header className="mb-8">
        <h1 className="text-lg font-black tracking-tight mb-1 text-slate-900 uppercase leading-none">After-Sales Audit</h1>
        <p className="text-slate-500 font-bold italic text-[10px] tracking-wide">Customer return requests and refund governance.</p>
      </header>
      ...
      <section className="bg-white border border-slate-200 rounded-3xl p-8 shadow-sm text-slate-900">
        <div className="flex items-center gap-3 mb-8">
           ...
           <h2 className="text-xs font-black uppercase tracking-tighter">Pending Returns</h2>
        </div>
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="border-b border-slate-100 italic">
                <th className="pb-4 text-[9px] font-black text-slate-400 uppercase tracking-widest">Order ID</th>
                <th className="pb-4 text-[9px] font-black text-slate-400 uppercase tracking-widest">Reason / Issue</th>
            ...
            <tbody className="divide-y divide-slate-50">
              {isLoading ? (
                <tr><td colSpan={4} className="py-8 text-center text-[10px] uppercase font-bold text-slate-400 font-mono">Scanning Return Logs...</td></tr>
                ...
                <tr key={returnReq._id || i} className="group hover:bg-slate-50/50 transition-all">
                  <td className="py-4">
                    <p className="text-[11px] font-black text-slate-900 leading-none mb-1">#{returnReq.orderId || `ORD-RET-${i}`}</p>
                    <p className="text-[9px] text-slate-400 font-bold tracking-tight italic">Customer: {returnReq.customerName || 'Syncing...'}</p>
                  </td>
                  <td className="py-4">
                     <div className="flex items-center gap-2">
                        <ShieldAlert size={12} className="text-orange-500" />
                        <span className="text-[10px] font-bold text-slate-600">{returnReq.reason || 'Not Specified'}</span>
                     </div>
                  </td>
                  <td className="py-4">
                ...
                <tr><td colSpan={4} className="py-12 text-center text-[10px] uppercase font-bold text-slate-400 italic">No return requests found in the ecosystem.</td></tr>
              )}
            </tbody>
          </table>
        </div>
