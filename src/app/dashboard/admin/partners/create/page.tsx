  ...
  return (
    <div className="max-w-2xl mx-auto">
      <header className="mb-8">
        <h1 className="text-lg font-black tracking-tight mb-1 text-slate-900 uppercase leading-none">Global Partner Onboarding</h1>
        <p className="text-slate-500 font-bold italic text-[10px] tracking-wide">Register a strategic partner for regional operations and growth.</p>
      </header>

      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="bg-white border border-slate-200 rounded-3xl p-8 shadow-sm">
          <h2 className="text-xs font-black border-l-2 border-emerald-600 pl-3 uppercase tracking-tighter mb-6">Partner Identity</h2>
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <input 
                  ...
                  className="w-full pl-10 pr-4 py-3 bg-slate-50 border border-slate-100 rounded-xl text-[11px] font-bold focus:outline-none focus:ring-2 focus:ring-emerald-600/20 transition-all"
                />
              </div>
            </div>
        ...
        <div className="bg-white border border-slate-200 rounded-3xl p-8 shadow-sm">
          <h2 className="text-xs font-black border-l-2 border-emerald-600 pl-3 uppercase tracking-tighter mb-6">Business Categories</h2>
          
          <div className="space-y-4 relative">
            <p className="text-[9px] text-slate-400 font-bold uppercase tracking-widest pl-1">Assign operational domains</p>
            ...
            <div className="relative">
                ...
                <div className="flex flex-wrap gap-1">
                  {selectedCategories.length > 0 ? (
                    selectedCategories.map(id => {
                      ...
                      return (
                        <span key={id} className="px-2 py-0.5 bg-emerald-100 text-emerald-700 rounded text-[9px] font-black uppercase">
                          {cat?.name || "..."}
                        </span>
                      );
                    })
                  ) : (
                    <span className="text-slate-400 italic font-medium">Select target categories...</span>
                  )}
                </div>
              ...
              {isDropdownOpen && (
                <div className="absolute z-50 w-full mt-2 bg-white border border-slate-100 rounded-2xl shadow-xl shadow-slate-200/50 max-h-60 overflow-y-auto p-2 space-y-1 animate-in fade-in zoom-in duration-200">
                  {isLoadingCategories ? (
                    <div className="flex items-center gap-2 text-slate-400 italic text-[10px] p-4">
                    ...
                    <div className="text-center py-4 text-[10px] text-slate-400 italic">No sectors available</div>
                  )}
                </div>
              )}
            </div>
